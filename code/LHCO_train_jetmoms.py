...
mjjmin = 2800
mjjmax = 7000
mybinboundaries = np.round(np.logspace(np.log10(mjjmin), np.log10(mjjmax), num=17))
...
mjj = bg_plus_signal[:,0]
mjj_bg = bg_plus_signal_bg[:,0]
bg_plus_signal = np.nan_to_num(bg_plus_signal)
bg_plus_signal = bg_plus_signal[np.greater(mjj,mjjmin) & np.less(mjj,mjjmax)]

bg_plus_signal_bg = np.nan_to_num(bg_plus_signal_bg)
bg_plus_signal_bg = bg_plus_signal_bg[np.greater(mjj_bg,mjjmin) & np.less(mjj_bg,mjjmax)]
...
bg_plus_signal_binned_bg = bin_data(bg_plus_signal_bg)
del bg_plus_signal
del bg_plus_signal_bg
...
model_utils_bg = {}
model_utils_bg[bin_i] = model_ensemble(bg_plus_signal_binned_bg, bin_i = bin_i, kfolds=myargs['-kfold'], preprocess = preprocess, eff_for_thresh = myargs['-checkeff'])
...
# if True:
    ...
    for k in range(startnum,myargs['-kfold']):
        ...
        for l in range(myargs['-kfold']):
            ...
            data_train_all = data_train_all[perms_train]
            labels_train_all = labels_train_all[perms_train]
            weights_train_all = weights_train_all[perms_train]
            data_valid_all = data_valid_all[perms_valid]
            labels_valid_all = labels_valid_all[perms_valid]
            weights_valid_all = weights_valid_all[perms_valid]
            ...
            for i in range(ntries):
                ...
                if (not myargs['-makeensemble']) & (not ((k == myargs['-startk']) & (l < myargs['-startl']))) & (k >= myargs['-startk']):
                    ...
                    model.add(Dense(512, input_dim=numvars,use_bias=True,
                                    #activation='relu',
                                    bias_initializer = keras.initializers.TruncatedNormal(mean=0., stddev=0.04)))
                    model.add(keras.layers.LeakyReLU(alpha=0.1))
                    model.add(Dropout(0.1))
                    model.add(Dense(512, use_bias=True, activation='elu',
