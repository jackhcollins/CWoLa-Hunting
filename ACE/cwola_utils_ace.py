...
class model_ensemble:
    ...
    def get_trainval_data(self,k,l, sideband = "both"):
        ...
        if (k != self.current_k) or (l != self.current_l) or (self.current_trainval_data == None):
            ...
            if sideband is not "high":
                ...
                low_sideband_train = np.zeros((0,len(train_dataset[0][0])))
                low_sideband_valid = np.zeros((0,len(train_dataset[0][0])))
            ...
            if sideband is not "low":
                ...
                high_sideband_train = np.zeros((0,len(train_dataset[0][0])))
                high_sideband_valid = np.zeros((0,len(train_dataset[0][0])))
            ...
            train_data = np.concatenate((low_sideband_train,signal_train,high_sideband_train),axis=0)            
            valid_data = np.concatenate((low_sideband_valid, signal_valid, high_sideband_valid), axis=0)
            
            categories_train = np.concatenate((np.zeros(n_low,dtype=np.int8),
                                               np.ones(n_signal,dtype=np.int8),
                                               np.zeros(n_high,dtype=np.int8)))
            
            categories_valid = np.concatenate((np.zeros(len(low_sideband_valid),dtype=np.int8),
                                              np.ones(len(signal_valid),dtype=np.int8),
                                              np.zeros(len(high_sideband_valid),dtype=np.int8)))
            
            weights_train = np.concatenate((np.ones(n_low)*0.25*n_total/max(n_low,1),
                                           np.ones(n_signal)*0.5*n_total/n_signal,
                                           np.ones(n_high)*0.25*n_total/max(n_high,1)))
            
            weights_valid = np.concatenate((np.ones(len(low_sideband_valid))*0.25*n_total/max(n_low,1),
                                           np.ones(len(signal_valid))*0.5*n_total/n_signal,
                                           np.ones(len(high_sideband_valid))*0.25*n_total/max(n_high,1)))
    ...
    def print_scatter_avg_allk_signalregion(self,axes_list=[[0,1]],axes_labels=None,
        ...
        AddPredictionsToScatter_nestedcrossval(data, predictions,
    ...
    def print_scatter_onemodel_signalplussidebandregion(self,k,l,i,axes_list=[[0,1]],
                                                        axes_labels=None,
                                                        rates = np.array([0.5,0.95,0.98,0.99]),
                                                        colors=['silver','grey','khaki','goldenrod','firebrick'],
                                                        plotmode = "save"):
...
def get_thresholds(predictions, rates, accuracy = None):
...
#     thresholds = [sorted_threshold_predictions[int(index)] for index in threshold_indices]
    ...
    thresholds = get_thresholds(threshold_predictions,rates,accuracy = accuracy)
...
#                             for i in range(0,len(extended_rates)-1)])
    ...
    for h, axes in enumerate(axes_list):
        ...
        if plotrange is not None:
            plt.xlim(plotrange[h,0])
            plt.ylim(plotrange[h,1])
...
class check_eff(keras.callbacks.Callback):
    ...
    def __init__(self,
                 verbose=0,                        # Verbose = 1: print data af each epoch.
                 ...
                 plot_period=99999,                # How frequently to plot performance metric
        ...
        self.save_plot_end = save_plot_end
...
#         if (not self.save_plot_end) and (not self.show_plot_end):
#             return
...
#         if len(self.effs_train_avg) > 0:
#             plt.plot(self.effs_train_avg,color='C1',linestyle='-')
...
#                     self.effs_train_avg.append(np.mean(np.array(self.effs_train)[-self.avg_length:]))
                ...
                if(len(self.effs_train) >= self.avg_length):
                    self.effs_train_avg = self.moving_average(self.effs_train)
        ...
        if (epoch % self.plot_period == 0) & (epoch > 0):
            plt.grid(b=True)
            plt.xlabel('Epoch')
            plt.ylabel('Sig. reg. eff at fixed sideband eff')
            plt.subplot(1, 2, 2)
...
def get_p_value(ydata,binvals,mask=[],verbose=0,plotfile=None,yerr=None,return_teststat = False,ylim=None):
